...
const WS_RECONNECT_ATTEMPTS = parseInt(import.meta.env.VITE_WS_RECONNECT_ATTEMPTS) || 5;
const WS_RECONNECT_DELAY = parseInt(import.meta.env.VITE_WS_RECONNECT_DELAY) || 3000;
const WS_HEARTBEAT_INTERVAL = parseInt(import.meta.env.VITE_WS_HEARTBEAT_INTERVAL) || 30000;
...
class WebSocketService {
  ...
  generateMockAccessHistoryUpdate() {
    return {
      ...
      data: {
        ...
        account: `realtime${Math.floor(Math.random() * 1000)}@example.com | 090${Math.floor(Math.random() * 10000000)} | Platform`,
  ...
  generateMockAutoLoginProgress() {
    return {
      ...
      status: ['waiting_otp', 'processing', 'completed', 'failed'][Math.floor(Math.random() * 4)],
      website: ['facebook.com', 'gmail.com', 'banking.vietcombank.com.vn'][Math.floor(Math.random() * 3)],
